<#
.SYNOPSIS
    Detection script for Dell Command | Configure (DCC)

.DESCRIPTION
    Checks the uninstall registry to see if Dell Command | Configure is installed
    and whether it is at least version 5.2.2.292. This avoids depending on a hard-coded GUID.

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\detect-dcc.ps1
...
$requiredVersion = [version]'5.2.2.292'
...
try {
    ...
    $logFile = Join-Path $logFolder 'dcc-detect.log'
}
catch {
    $logFolder = $env:TEMP
    $logFile = Join-Path $logFolder 'dcc-detect.log'
...
try {
    Write-Log "=== Starting Dell Command | Configure detection ==="
    ...
    foreach ($path in $paths) {
        ...
        foreach ($item in $items) {
            ...
            if (($displayName -match 'Dell Command \| Configure') -or ($displayName -match 'Dell Command Configure') -or ($displayName -match 'DCC')) {
                if ($displayVersion) {
    ...
    Write-Log "Dell Command | Configure not detected or not compliant" -Level Warning
    exit 1
}
catch {
