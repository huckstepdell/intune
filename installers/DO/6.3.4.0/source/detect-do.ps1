<#
.SYNOPSIS
    Detection script for Dell Optimizer (DO)

.DESCRIPTION
    Checks the uninstall registry to see if Dell Optimizer is installed
    and whether it is at least version 6.3.4.0. This avoids depending on a hard-coded GUID.

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\detect-do.ps1
...
$requiredVersion = [version]'6.3.4.0'
...
try {
    ...
    $logFile = Join-Path $logFolder 'do-detect.log'
}
catch {
    $logFolder = $env:TEMP
    $logFile = Join-Path $logFolder 'do-detect.log'
...
try {
    Write-Log "=== Starting Dell Optimizer detection ==="
    ...
    foreach ($path in $paths) {
        ...
        foreach ($item in $items) {
            ...
            if ($displayName -match 'Dell Optimizer') {
                if ($displayVersion) {
    ...
    Write-Log "Dell Optimizer not detected or not compliant" -Level Warning
    exit 1
}
catch {
