<#
.SYNOPSIS
    Detection script for Dell Display and Peripheral Manager (DDPM)

.DESCRIPTION
    Checks the uninstall registry to see if Dell Display and Peripheral Manager is installed
    and whether it is at least version 2.3.0.17. This avoids depending on a hard-coded GUID.

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\detect-ddpm.ps1
...
$requiredVersion = [version]'2.3.0.17'
...
try {
    ...
    $logFile = Join-Path $logFolder 'ddpm-detect.log'
}
catch {
    $logFolder = $env:TEMP
    $logFile = Join-Path $logFolder 'ddpm-detect.log'
...
try {
    Write-Log "=== Starting Dell Display and Peripheral Manager detection ==="
    ...
    foreach ($path in $paths) {
        ...
        foreach ($item in $items) {
            ...
            if (($displayName -match 'Dell Display and Peripheral Manager') -or ($displayName -match 'Dell Peripheral Manager') -or ($displayName -match 'DDPM')) {
                if ($displayVersion) {
    ...
    Write-Log "Dell Display and Peripheral Manager not detected or not compliant" -Level Warning
    exit 1
}
catch {
