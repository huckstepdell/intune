<#
.SYNOPSIS
    Detection script for ASP.NET Core Runtime 10.x

.DESCRIPTION
    Uses 'dotnet --list-runtimes' to check whether Microsoft.AspNetCore.App
    is installed at least at version 10.0.11 within the 10.x band. Runtimes are
    side-by-side, and this app may be superseded by a later 10.x patch applied
    through Dell Command | Update or Windows Update, which should still be compliant.

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\detect-dotnet-aspnetcore.ps1
...
$requiredVersion = [version]'10.0.11'
$majorVersion = 10
...
try {
    ...
    $logFile = Join-Path $logFolder 'dotnet-aspnetcore-10-detect.log'
}
catch {
    $logFolder = $env:TEMP
    $logFile = Join-Path $logFolder 'dotnet-aspnetcore-10-detect.log'
...
try {
    Write-Log "=== Starting ASP.NET Core Runtime $majorVersion.x detection ==="
    ...
    Write-Log "ASP.NET Core Runtime $majorVersion.x not detected or not compliant" -Level Warning
    exit 1
}
catch {
