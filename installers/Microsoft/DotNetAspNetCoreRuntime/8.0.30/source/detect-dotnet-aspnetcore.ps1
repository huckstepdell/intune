<#
.SYNOPSIS
    Detection script for ASP.NET Core Runtime 8.x

.DESCRIPTION
    Uses 'dotnet --list-runtimes' to check whether Microsoft.AspNetCore.App
    is installed at least at version 8.0.30 within the 8.x band. Runtimes are
    side-by-side, and this app may be superseded by a later 8.x patch applied
...
$requiredVersion = [version]'8.0.30'
$majorVersion = 8
...
try {
    ...
    $logFile = Join-Path $logFolder 'dotnet-aspnetcore-8-detect.log'
}
catch {
    $logFolder = $env:TEMP
    $logFile = Join-Path $logFolder 'dotnet-aspnetcore-8-detect.log'
